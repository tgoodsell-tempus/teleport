...
func TestIntegrationResource(t *testing.T) {
	...
	t.Run("get", func(t *testing.T) {
		...
		for i := 0; i < totalIntegrations; i++ {
			...
			err = auth.GetAuthServer().CreateIntegration(ctx, ig1)
			require.NoError(t, err)
		}
		...
		t.Run("get specific integration with human output", func(t *testing.T) {
			...
			require.NoError(t, err)
			outputString := buff.String()
			require.Contains(t, outputString, "AWSRoleARN=arn:aws:iam::123456789012:role/OpsTeam")
	...
	t.Run("create", func(t *testing.T) {
		...
		// Update the AWSRoleARN to another role
		...
		// The AWSRoleARN must be updated
		buff, err = runResourceCommand(t, fileConfig, []string{"get", "integration/myawsint", "--format=text"})
		require.NoError(t, err)
		outputString = buff.String()
		require.Contains(t, outputString, "AWSRoleARN=arn:aws:iam::123456789012:role/DevTeam")
	})
}
...
metadata:
  name: myawsint
spec:
  aws_role_arn: "arn:aws:iam::123456789012:role/OpsTeam"
`
)
