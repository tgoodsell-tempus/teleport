...
func (integrationsExecutor) getAll(ctx context.Context, cache *Cache, loadSecrets bool) ([]types.Integration, error) {
	...
	for {
		var assignments []types.Integration
		var err error
		assignments, startKey, err = cache.Integrations.ListIntegrations(ctx, 0, startKey)
		if err != nil {
			return nil, trace.Wrap(err)
		}
...
func (integrationsExecutor) upsert(ctx context.Context, cache *Cache, resource types.Integration) error {
	err := cache.integrationsCache.CreateIntegration(ctx, resource)
	if trace.IsAlreadyExists(err) {
		err = cache.integrationsCache.UpdateIntegration(ctx, resource)
	}
	return trace.Wrap(err)
}
