---
title: Configure Teleport to Automatically Enroll Azure Virtual Machines (Preview)
...
execute an install script on these discovered instances that will install
Teleport, start it and join the cluster.

In this scenario, Teleport Discovery Service uses an Azure invite token with a
...
managed identities.
- Azure virtual machines running Ubuntu/Debian/RHEL if making use of the
...
(!docs/pages/includes/server-access/azure-configure-service-principal.mdx!)
...
Save the generated token in `/tmp/token` on the Node (virtual machine) that will
...
change what gets executed when enrolling discovered Azure instances.
...
(!docs/pages/includes/server-access/azure-troubleshooting.mdx!)

## Next steps

- Read [Joining Nodes via Azure Managed Identity](../../management/guides/joining-nodes-azure.mdx)
