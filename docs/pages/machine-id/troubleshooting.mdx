...
Non-TLS routing configured proxies use specific ports for types of traffic. That requires that a Kubernetes
connection use its designated port. Currently Machine ID requires the Kubernetes public address is set
