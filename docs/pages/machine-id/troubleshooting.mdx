...
The `kube_public_addr` is set within the Teleport `proxy_service` configuration by
administrators. The proxy will require a restart after the configuration is updated.
...
`kube_public_address` is set within `proxy.kube.public_addr`.
