...
A self-hosted Teleport cluster is connecting Machine ID to Kubernetes clusters with the following errors.  This can happen for non-TLS configured 
Teleport clusters.
...
To confirm the TLS routing mode check the value of the `proxy.tls_routing_enabled` key with this command, substituting your proxy address:
...
connection use its designated port. Currently Machine ID requires that the Kubernetes public address is set
to use the correct port. Otherwise it will use the Proxy web port which can cause these
type of errors.

### Resolution

The `kube_public_addr` is set within the Teleport `proxy_service` configuration by administrators. 
The proxy will require a restart after the configuration is updated.
...
Retrieve the configuration listing from the proxy web address to confirm the `kube_public_address` is set 
within `proxy.kube.public_addr`.
