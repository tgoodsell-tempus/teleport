...
// cluster stores the name of the cluster
...
func (c *cluster) set(name string) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// clusterCredentials is a [credentials.TransportCredentials] that obtains the name
// of the cluster being connected to from the certificate presented by the server.
// This allows us to determine the cluster name when using jump hosts.
...
func newGRPCClient(ctx context.Context, cfg *ClientConfig) (_ *Client, err error) {
	...
	conn, err := grpc.DialContext(
		...
		cfg.ProxySSHAddress,
		append(cfg.DialOpts,
			grpc.WithTransportCredentials(&clusterCredentials{TransportCredentials: cfg.creds(), cluster: c}),
...
const (
	...
	teleportAuthority = "x-teleport-authority"
)

// clusterCallback is a [ssh.HostKeyCallback] that obtains the name
// of the cluster being connected to from the certificate presented by the server.
// This allows us to determine the cluster name when using jump hosts.
func clusterCallback(c *cluster, cb ssh.HostKeyCallback) ssh.HostKeyCallback {
	return func(hostname string, remote net.Addr, key ssh.PublicKey) error {
		...
		c.set(cert.Permissions.Extensions[teleportAuthority])
		return nil
	}
}
